...
class DeformFuserv2(nn.Module):
    ...
    def __init__(self, in_nc, out_nc, nf, nb, base_ks=3, deform_ks=3):
        ...
        Args:
            ...
            deform_ks: size of the deformable kernel.
        """
        super(DeformFuserv2, self).__init__()
        ...
        # deformable conv
        self.deform_conv = DeformConv2d(in_nc,2*in_nc,base_ks,padding=base_ks//2,groups=in_nc)

        # output proj
        self.output_proj = nn.Sequential(
            nn.Conv2d(2*in_nc, out_nc, base_ks, padding=base_ks//2),
            nn.ReLU(inplace=True)
            )
